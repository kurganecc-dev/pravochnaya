...
Бриф переработан в производственный мастер из шести этапов:

1. проект, контакты и согласование;
2. задача, главная мысль и аудитория;
3. содержание, стилистика, сценарий, графика и озвучка;
4. организация съёмки;
5. итоговые форматы и сроки;
6. бюджет, материалы и порядок передачи правок.

Форма показывает только актуальные уточнения. Например:

- вопросы о площадке и участниках появляются только при новой съёмке;
- при выборе диктора открываются требования к голосу и тексту;
- при выборе инфографики запрашиваются данные и элементы для анимации;
- готовый сценарий, черновик и тезисы требуют ссылки или описания;
- языковые версии открывают уточнение по переводу и субтитрам.

В правой колонке автоматически формируются:

- процент готовности к предварительной оценке;
- список недостающих обязательных данных с быстрым переходом к вопросу;
- краткая сводка проекта;
- полный структурированный текст для отправки продакшену.

Бриф сохраняется локально, копируется одной кнопкой и скачивается как TXT. Черновики старой 16-вопросной формы автоматически переносятся в новую структуру.

## Стартовый экран сервисов

При первом открытии пользователь видит два понятных сценария:

- **Заполнить бриф** — для заказа новой съёмки или ролика;
- **Оставить правки** — для комментариев по таймкодам и экспорта в Premiere Pro.

После выбора сервис запоминает последний раздел. Стартовый экран всегда можно открыть повторно кнопкой **«Сервисы»** в верхней панели. Прямые ссылки `#brief` и `#review` сразу открывают соответствующий инструмент.
